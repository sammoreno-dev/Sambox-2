...
_get_cache_targets_catalog() {
    # Garante que as variáveis do sistema (como HOME e USER) expandam perfeitamente dentro do Here-Doc
    cat << EOF
...
wine|Temporários do Sistema Wine|${HOME}/.wine/drive_c/windows/temp
wine|Temporários do Usuário Wine AppData|${HOME}/.wine/drive_c/users/${USER}/AppData/Local/Temp
wine|Temporários Raiz do Usuário Wine|${HOME}/.wine/drive_c/users/${USER}/Temp
lutris|Cache do Lutris|${HOME}/.cache/lutris
heroic|Cache de Shaders do Heroic Launcher|${HOME}/.cache/heroic
heroic|Cache de Lojas do Heroic Launcher|${HOME}/.config/heroic/store_cache
EOF
}
