#!/usr/bin/env bash
# Sambox 2 - Sub-módulo de Auditoria Criptográfica de BIOS
...
KNOWN_BIOS=(
    ...
    "dc_boot.bin|Dreamcast|S Sega Dreamcast Boot ROM|e10c53c2f8b90bab96eed2e96717013898c6d123"
...
retro_verify_bios() {
    local bios_dir="${1:-${HOME}/RetroROMs/bios}"
    local has_sha1=0
    ...
    for item in "${KNOWN_BIOS[@]}"; do
        ...
        if [[ -f "${fullpath}" ]]; then
            ...
            if [[ ${has_sha1} -eq 1 ]]; then
                # Obtém o hash atual e extrai apenas a primeira coluna de forma segura
                local current_sha1
                current_sha1=$(sha1sum "${fullpath}" 2>/dev/null | awk '{print $1}')
                
                # Comparação direta usando strings em minúsculo nativo do Bash
            ...
            # Corrigido: usando %b no final e garantindo que as tags DIM/RST interpretem corretamente
